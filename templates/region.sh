...
kubectl create configmap cross-layer-config \
  --from-literal=wavelength_endpoint=172.17.0.1 \
...
MQTT_BROKER = "35.182.247.43"  # or public IP if hosted outside cluster
MQTT_PORT = 1883
TOPIC = "pipeline/final/out"

def on_connect(client, userdata, flags, rc):
    print(f"✅ Cloud connected to MQTT broker (code {rc})")
    client.subscribe(TOPIC)

def on_message(client, userdata, msg):
    print(f"📥 Received processed audio on {msg.topic} ({len(msg.payload)} bytes)")
    filename = f"final_{uuid.uuid4().hex}.mp3"
    with open(filename, "wb") as f:
        f.write(msg.payload)
    print(f"💾 Saved final audio as: {filename}")
